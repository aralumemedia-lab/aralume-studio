...
type ApiMeta             = { requestId: string; generatedAt: string; page?: number; pageSize?: number; total?: number };
```
...
- `PerformanceMetric`, `CostEntry`, `AuditLog`, `OperationalModePolicy`
...
| GET    | /api/content/production             | `ProductionItem[]`            |
| GET    | /api/ideas                          | `ContentIdea[]`               |
| GET    | /api/research                       | `ResearchSession[]`           |
