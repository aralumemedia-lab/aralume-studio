...
Sprint 14 closed with `V1.0 NAO ACEITA`. The matrix still shows the editorial pipeline gaps concentrated on V1-03 to V1-06, with V1-02 as an upstream dependency only. The repository already has real backend/editorial routes, real frontend routes for `/ideas`, `/research`, `/scripts`, and an editorial contract layer, but the current frontend does not yet prove the complete operator flow from idea creation to visual plan and scene planning.
...
V1-02 is a dependency on the channel surface, not part of the first remediation epic scope.
...
1. Select a channel.
2. Create a content idea.
3. Create a research session.
4. Register sources.
5. Register claims.
6. Create a script.
7. Create a new script version.
8. Review history.
9. Create a visual plan.
10. Add and order scenes.
11. Reload the app.
12. Confirm persistence.
13. Confirm auditability.
14. Confirm channel isolation.
...
- Frontend entry points for ideas, research, scripts and the visual-planning surface.
