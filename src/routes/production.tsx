...
export const Route = createFileRoute("/production")({
  head: () => ({
    meta: [
      { title: "Produção — Aralume" },
      { name: "description", content: "Fila de produção editorial multiagentes." },
    ],
  }),
  component: function ProductionPage() {
    const { activeChannelId } = useChannelContext();
    const q = useQuery({
      queryKey: ["prod-page", activeChannelId],
    ...
    return (
      <div>
        <PageHeader
          eyebrow="Operação"
          title="Produção"
          description="Todos os conteúdos em andamento, com etapa, agente responsável e próxima ação."
        />
        <div className="p-4">
          <Card padded={false}>
            <CompactTable
              ...
              empty="Nada em produção no canal ativo."
            />
          </Card>
        </div>
      </div>
