...
import { formatCurrencyCents, formatDateTime, formatDuration, formatNumber, formatRelative } from "@/lib/format";
import { cn } from "@/lib/utils";
import type { AgentDefinition, AgentHandoff, AgentPhase, AgentRun, WorkflowRun } from "@/contracts/types";

export const Route = createFileRoute("/agent-office")({
  head: () => ({
    meta: [
      { title: "Escritório de Agentes — Aralume" },
      { name: "description", content: "Cockpit operacional dos agentes editoriais da Aralume: execução, handoffs, bloqueios e custos." },
...
function AgentOfficePage() {
  ...
  const snapQ = useQuery({ queryKey: ["office", activeChannelId], queryFn: () => getAgentOfficeSnapshot(activeChannelId) });
  ...
  const avgDur = agents.filter((a) => a.durationSeconds).reduce((s, a) => s + (a.durationSeconds ?? 0), 0) / Math.max(1, agents.filter((a) => a.durationSeconds).length);
    ...
    <div>
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-3">
          <KpiCard label="Agentes ativos" value={formatNumber(agents.length)} icon={<Bot size={14} />} />
          <KpiCard label="Executando" value={formatNumber(runningCount)} tone="info" />
          <KpiCard label="Bloqueados" value={formatNumber(blockedCount)} tone={blockedCount > 0 ? "critical" : undefined} />
          <KpiCard label="Handoffs ativos" value={formatNumber(handoffs.filter((h) => h.status === "pending" || h.status === "delivered").length)} />
          <KpiCard label="Itens na fila" value={formatNumber(agents.filter((a) => a.status === "idle" || a.status === "waiting_input").length)} />
          <KpiCard label="Falhas" value={formatNumber(failedCount)} tone={failedCount > 0 ? "warning" : undefined} />
        ...
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-12 xl:col-span-9 space-y-4">
            <Card padded={false}>
              ...
              <div className="overflow-x-auto scrollbar-thin px-4 pb-4">
                <div className="grid gap-3 min-w-[1200px]" style={{ gridTemplateColumns: `repeat(${PHASES.length}, minmax(180px, 1fr))` }}>
                  {PHASES.map((p) => (
                    <PhaseColumn
                      key={p.key}
                      phase={p}
            ...
            <Card padded={false}>
              <div className="p-4 pb-2">
                <SectionHeader title="Handoffs recentes" description="Entregas de artefatos entre agentes." />
              </div>
            ...
            <Card padded={false}>
              <div className="p-4 pb-2">
                <SectionHeader title="Workflows em execução" description="Timeline consolidada de execução." />
              </div>
          ...
          <div className="col-span-12 xl:col-span-3 space-y-4">
            <Card>
              <SectionHeader title="Detalhe do agente" description="Selecione um card do board para inspecionar." />
              {selectedAgentRun ? (
                <div className="space-y-3">
                  <div>
                    <div className="text-[10.5px] uppercase tracking-[0.08em] text-muted-foreground">Agente</div>
                    <div className="text-sm font-semibold">{selectedAgentRun.agentName}</div>
                    <div className="text-[11.5px] text-muted-foreground">{selectedAgentRun.currentTask}</div>
                  ...
                  <div className="space-y-1.5 text-[11.5px]">
                    ...
                    <Row label="Custo estimado" value={formatCurrencyCents(selectedAgentRun.costEstimateCents)} />
                    <Row label="Custo atual" value={formatCurrencyCents(selectedAgentRun.costActualCents)} />
                    <Row label="Duração" value={selectedAgentRun.durationSeconds ? formatDuration(selectedAgentRun.durationSeconds) : "—"} />
                    <Row label="Iniciado" value={formatDateTime(selectedAgentRun.startedAt)} />
                    <Row label="Última atividade" value={formatRelative(selectedAgentRun.lastActivityAt)} />
                  </div>
                  <div>
                    <div className="text-[10.5px] uppercase tracking-[0.08em] text-muted-foreground mb-1">Progresso</div>
                    <ProgressBar value={selectedAgentRun.progressPercent} tone={selectedAgentRun.status === "blocked" ? "critical" : "info"} />
                  </div>
            ...
            <Card padded={false}>
              <div className="p-4 pb-2">
                <SectionHeader title="Bloqueios" description="Itens travados aguardando decisão ou correção." action={<ShieldAlert size={14} className="text-warning" />} />
              </div>
                ...
                {blocked.map((b) => (
                  <div key={b.id} className="px-4 py-2.5">
                    ...
                    <div className="mt-1.5 flex items-center justify-between">
                      ...
                      <span className="text-[10.5px] text-muted-foreground">{formatRelative(b.createdAt)}</span>
                    </div>
                  </div>
                ))}
                {blocked.length === 0 && <div className="p-4"><EmptyState title="Nenhum bloqueio ativo" /></div>}
              </div>
            </Card>
          </div>
        </div>
  ...
  return (
    <div className="rounded-md border border-border bg-surface-muted">
      <div className="px-2.5 py-2 border-b border-border flex items-center justify-between">
        <span className="text-[10.5px] uppercase tracking-[0.08em] text-muted-foreground font-medium">{phase.label}</span>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                run ? "border-border hover:border-primary/50 hover:bg-accent/30" : "border-dashed border-border/70 opacity-60",
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <div className="text-[10.5px] text-muted-foreground truncate">{def.description}</div>
                </div>
              </div>
              {run ? (
                <>
                  <div className="mt-2 text-[10.5px] text-muted-foreground truncate">{run.currentTask}</div>
                  <div className="mt-1.5"><ProgressBar value={run.progressPercent} tone={run.status === "blocked" ? "critical" : run.status === "failed" ? "critical" : run.status === "waiting_input" || run.status === "waiting_approval" ? "attention" : "info"} /></div>
                  <div className="mt-1.5 flex items-center justify-between gap-1">
                    <AgentStatusBadge status={run.status} />
                    <span className="text-[10.5px] tabular-nums text-muted-foreground">{formatCurrencyCents(run.costActualCents)}</span>
                  </div>
                </>
              ) : (
...
const handoffColumns = (defs: AgentDefinition[]): Column<AgentHandoff>[] => {
  ...
  return [
    { key: "flow", header: "Fluxo", render: (r) => (
    ...
    { key: "artifact", header: "Artefato", render: (r) => <span className="text-muted-foreground">{r.artifactType}</span> },
    { key: "title", header: "Título", render: (r) => <span className="truncate">{r.title}</span> },
    { key: "status", header: "Status", render: (r) => <span className={cn("text-[11px] uppercase tracking-wide", r.status === "delivered" ? "text-ok" : r.status === "blocked" ? "text-critical" : "text-attention")}>{r.status}</span> },
...
const workflowCols: Column<WorkflowRun>[] = [
  ...
  { key: "at", header: "Atividade", render: (r) => <span className="text-muted-foreground">{formatRelative(r.lastActivityAt)}</span> },
];
